...
def my_parse_youtube_umass(path):
    ...
    """
    helper = lambda x: sum([256 ** j * int(i) for j, i in enumerate(x.split('.')[::-1])])
    with open(path) as f:
        for line in f:
            entry = line.split(" ")
            timestamp = entry[0]
            youtube_server_addr = entry[1]
            client_addr = helper(entry[2])
            request = entry[3]
            video_id = ord(entry[4][0]) * 1000 + ord(entry[4][1]) * 100 + ord(entry[4][2]) * 10 + ord(entry[4][3])
            content_server_addr = entry[5]
            yield dict(
                time=timestamp,
                youtube_server_addr=youtube_server_addr,
