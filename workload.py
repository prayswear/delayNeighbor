...
class StationaryWorkload(object):
    ...
    def __init__(self, topology, n_contents, alpha, rate=1.0,
                 n_warmup=10 ** 5, n_measured=4 * 10 ** 5, seed=None, **kwargs):
        if alpha < 0:
            raise ValueError('alpha must be positive')
        self.requester = [v for v in topology.nodes()]
    ...
    def all_events(self):
        # {timestamp:(pos,level,content,log)}

        pass

    def single_level_events(self, level):
        # {timestamp:(pos,level,content,log)}
        req_counter = 0
        t_event = 0.0
        events = []
        while req_counter < self.n_warmup + self.n_measured:
            temp = (random.expovariate(self.rate)) * 1000
            t_event += temp
            requester = random.choice(self.requester)
            content = int(self.zipf.rv())
            log = (req_counter >= self.n_warmup)
            event = {'timestamp': t_event, 'pos': requester, 'content': content, 'level': level, 'log': log}
            events.append(event)
            req_counter += 1
        return events
...
class GlobetraffWorkload(object):
    ...
    def __init__(self, topology, reqs_file, contents_file, beta=0, **kwargs):
        ...
        if beta != 0:
            ...
            degree[iter(topology.adj[x]).next()], reverse=True)
...
class YOUTUBE_TRACE(object):
    ...
    def __init__(self, topology, n_contents, rate=1.0,
                 n_warmup=10 ** 3, n_measured=4 * 10 ** 5, **kwargs):
        self.receivers = [v for v in topology.nodes()
                          if topology.node[v]['stack'][0] == 'receiver']
        self.n_contents = n_contents
        self.contents = range(1, n_contents + 1)
        ...
        self.n_warmup = n_warmup
        self.n_measured = n_measured
        self.my_len = len(self.receivers)

    def __iter__(self):
        req_counter = 0
        t_event = 0.0
        mypath = "resources/youtube.parsed.031208.S1.txt"
        mypath1 = "resources/youtube.parsed.031208.24.txt"
        for n in traces.my_parse_youtube_umass(mypath):
            t_event += (random.expovariate(self.rate)) * 1000
            receiver = self.receivers[n['client_addr'] % self.my_len]
            content = n['video_id'] % self.n_contents + 1
            log = (req_counter >= self.n_warmup)
            event = {'receiver': receiver, 'content': content, 'log': log}
            yield t_event, event
            req_counter += 1
            if req_counter >= self.n_measured + self.n_warmup:
                break

        # for n in traces.my_parse_youtube_umass(mypath1):
        #     t_event += (random.expovariate(self.rate)) * 1000
        #     receiver = self.receivers[n['client_addr'] % self.my_len]
        #     content = n['video_id'] % self.n_contents + 1
        #     log = (req_counter >= self.n_warmup)
